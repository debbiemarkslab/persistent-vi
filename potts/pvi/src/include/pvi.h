...
enum {
    ...
    INFER_MAP,
    /* Maximum pseudolikelihood (MAP) */
    INFER_PLM,
    /* Bayesian posterior mean */
    INFER_BAYES,
    /* Variational approximation of posterior by a Gaussian */
    INFER_VBAYES,
    /* Heuristically determine hyperparameters for MAP estimation */
    INFER_HYBRID
};
...
/* Hierarchical priors for sparse variational learning  */
enum {
    /* Exponentially distributed standard deviation, marginally Laplacian */
    PRIOR_EXPONENTIAL,
    /* Inverse-Gamma distributed variances, marginally Student-t */
    PRIOR_IGAMMA,
    /* Half-Cauchy distributed standard deviation, marginally Horseshoe */
    PRIOR_HALFCAUCHY,
    /* Stacked Half-Cauchy distributed standard deviation, marginally Horseshoe+ */
    PRIOR_HALFCAUCHYPLUS
};
...
typedef struct {
    ...
    int noncentered;
    int hyperprior;
    int priorNGlobalParams;
    int maxIter;
    int gChains;
    int gSweeps;
    int vSamples;            /* Number of samples for KL stochastic gradients */
    ...
    /* Bayesian estimation of Lambda_h */
    int bayesLH;
} options_t;
...
/* Estimates effective sample size by simulation */
...
#define ungapij(i, j)           ali->ungapij[(i < j ? ((j)*(j - 1)/2 + i) : ((i)*(i - 1)/2 + j))]
