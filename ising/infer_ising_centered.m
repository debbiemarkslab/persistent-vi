...
gradfun = @(theta, i) grad_ELBO(theta, @ising_gradNLP_centered, ...
    num_samples);
...
params.sig_h = transform_sigma(logsig_nch);
params.sig_J = transform_sigma(logsig_ncJ);
params.sig_logzh = transform_sigma(logsig_logzh);
params.sig_logzJ = transform_sigma(logsig_logzJ);

function gradtheta = ising_gradNLP_centered(params)
...
%  Persistent Markov Chains
    ...
    [hyper, h, J, logzh, logzJ] = unpack_params(params, N, num_global);
    ...
    gradh = gradh - h ./ exp(2 * logzh);
    gradJ = gradJ - J ./ exp(2 * logzJ);
    gradlogzh = -1 + h.^2 ./ exp(2 * logzh);
    gradlogzJ = -1 + J.^2 ./ exp(2 * logzJ);
    ...
    gradlogzJ = gradlogzJ - diag(diag(gradlogzJ));
    gradtheta = pack_params(-gradhyper, -gradh, -gradJ, -gradlogzh, -gradlogzJ);
end

function callback(theta, ~, iter)
% Optionally plot current state during inference
    if ~strcmp(plot_mode, 'off') && mod(iter, 100) == 0
        [mu_hyp, mu_h, mu_J, mu_logzh, mu_logzJ] = ...
            unpack_params(theta(1:end/2), N, num_global);
        [logsig_hyp, logsig_h, logsig_J, logsig_logzh, logsig_logzJ] = ...
            unpack_params(theta(end/2 + [1:end/2]), N, num_global);
